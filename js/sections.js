...
(function () {
  ...
  function renderLogistika() {
    var rows = ROUTE.map(function (slug, index) {
      ...
      return [
        ...
        H.escape(c.logistics.bus),
        H.escape(c.logistics.walk)
      ];
    });
    H.paint('<h1>Логистика маршрута</h1>' +
      ...
      H.table(["Остановка", "От Гродно", "От Мостов", "Автобус", "Пешком"], rows) +
      ...
      '<section class="card"><h2>Автостанция «Мосты» и пригородные рейсы</h2>' +
      '<dl class="fact-panel"><dt>Адрес</dt><dd>' + H.escape(BUS.address) + "</dd>" +
      "<dt>Касса</dt><dd>" + H.escape(BUS.phone) + "</dd>" +
      "<dt>Часы работы</dt><dd>" + H.escape(BUS.hours) + "</dd></dl>" +
      "<p>" + H.escape(BUS.note) + "</p>" +
      '<p class="data-note">Источник: ' + H.escape(BUS.source) + "</p></section>" +
  ...
  function renderSpravka() {
    ...
    var places = FOOD.places.map(function (place) {
      return "<li>" + H.escape(place.name) + " — " + H.escape(place.address) + "</li>";
    }).join("");
    H.paint('<h1>Справочная информация</h1>' +
      ...
      '<section class="card"><h2>Где поесть</h2><p>' + H.escape(FOOD.note) + '</p><ul class="facts-list">' + places + "</ul></section>" +
